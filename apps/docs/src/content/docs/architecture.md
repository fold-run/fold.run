...
Task ownership is bound to the minting principal: another caller's task-scoped calls answer the same "unknown id" error as a task that doesn't exist, with no existence leak. The ownership index lives in shared state rather than process memory, so with Redis a whole fleet agrees on it and it survives a restart — landing on a different instance is not a way around the binding. Tasks fold has no ownership record for — minted out of band, expired after 24 hours, or on a fleet running without shared state — stay reachable by any caller via the probe fallback, and anonymous callers share one owner bucket, so no-auth deployments are unaffected.
