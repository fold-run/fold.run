...
import { BadgeCheck, Building2, Gauge, Globe, KeyRound, Network, PackageCheck, Radar, Shield, Terminal } from 'lucide-astro';
...
<CardGrid>
  ...
  <Card title="Protect fragile services" icon={Gauge}>
    ...
    a [budget](/consumption/) accumulates across a calendar period, so a
    busy month meets a ceiling.
  </Card>
  <Card title="Govern each customer separately" icon={Building2}>
    ...
    carries the tenant's name. [Tenancy →](/tenancy/)
  </Card>
  <Card title="Federate local servers" icon={Terminal}>
    ...
    strategy, guard, and policy rule applied unchanged. [Stdio servers →](/stdio/)
  </Card>
...
server, then [add governance](/configuration/) as the surface grows. Nothing
