...
| `GET /health` | always | Pings every upstream concurrently (5 s internal budget); `503` when none is reachable. The fan-out is single-flighted and reused for a second, so polling this unauthenticated endpoint in a loop can't multiply into upstream traffic (a reload or discovery sync invalidates it immediately). `/healthz` — the path through v1.4, kept as a deprecated alias through v1.8 — was **removed in v1.9**; a probe left on it now `404`s. Detailed fields (URLs, owners, labels, error text) appear only when auth is disabled, so an unauthenticated caller on a public deployment cannot enumerate the federation. Multi-endpoint upstreams include a per-replica `endpoints` array with the balancer's rotation state. |
| `GET /metrics` | always | Prometheus exposition (below). |
| `GET /api/federation` | `server.introspection.enabled` | The federation snapshot — upstream health and topology, each upstream's source (static vs discovered) and credential-strategy *name*, discovery status, policy shape, audit sink types, shared-state and tracing facts, and the viewer's tenant governance. Authenticates like `/mcp` and shares its rate budgets. Was `GET /console/api/state` through v1.8. |
| `GET /api/auth-hint` | `server.introspection.enabled` | The deliberately unauthenticated sign-in hint — issuer, client id, scopes, resource — read by a browser client before it holds a token. Public SPA configuration only. Was `GET /console/api/auth` through v1.8. |
| `GET /console/` | `server.console.enabled` | The read-only [console](/console/) page. Its assets are open (they carry no data); the dashboard reads `/api/federation` like any other client of it, so the page also requires `server.introspection.enabled`. |
