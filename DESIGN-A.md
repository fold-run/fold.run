...
### Primary
- **Route** `#5560ff` — every primary action, active diagram paths, focus rings.
  On dark grounds only (3.9:1 on Backplane as large/bold graphic color; never
  small text on Paper).
- **Route Deep** `#3c46e0` — the same indigo family on light grounds (docs light
  theme text-accent, 5.6:1 on Paper); hover/active depth on dark.
- **Carrier** `#c4c8ff` — links, badges, kicker labels, secondary emphasis.
...
Archivo (display + body) and Martian Mono (all mono) — user re-direct 2026-08-06 replacing the captured system stacks. Prototypes @import Google Fonts; migrate self-hosts woff2 with font-display: swap.
